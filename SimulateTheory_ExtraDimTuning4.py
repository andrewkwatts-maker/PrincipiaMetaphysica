...
"""
SimulateTheory_ExtraDimTuning_V4.py - Extra Dimension Influence Parameter Tuning

This script returns to the V2 physical model, but increases the penalty weight
on the Proton Decay Lifetime (τₚ) to force the optimizer to resolve the final,
most significant tension.
...
CRUCIAL MODIFICATION (V4):
---------------------------------
1. Reverted the sign of Δ(1/α_GUT) back to V2 (negative).
2. Increased the penalty weight for the τₚ term in the objective function to 4.0.
...
print("=" * 80)
print("PRINCIPIA METAPHYSICA - EXTRA DIMENSION TUNING SIMULATION (V4)")
print("Prioritizing τₚ Resolution (Weight = 4.0)")
print("=" * 80)
print()
...
# ==============================================================================
# FUNCTIONS: THEORY PREDICTIONS (PHYSICS REVERTED TO V2)
# ==============================================================================
...
def compute_alpha_GUT(alpha_4, alpha_5):
    ...
    V4 MODIFICATION: Sign of delta_alpha reverted to NEGATIVE (V2 logic).
    This ensures that a positive (α₄ + α₅) slightly REDUCES 1/α_GUT.
    """
    alpha_GUT_inv_base = 24.68
    
    # Extra dimension modifies running via KK tower (V2/Original sign)
...
# ==============================================================================
# OBJECTIVE FUNCTION: MINIMIZE DEVIATIONS FROM TARGETS (WEIGHTING MODIFIED HERE)
# ==============================================================================
...
def objective_function(params):
    ...
    V4 Modification: Increased τₚ penalty weight.
    """
    ...
    # --- WEIGHTS ---
    CHI2_WEIGHT_TAU_P = 4.0  # NEW: High weight to force resolution
    CHI2_WEIGHT_THETA_23 = 2.0 # Retained from V2/V3
    CHI2_WEIGHT_W0 = 1.0
    CHI2_WEIGHT_M_KK = 0.5

    # 1. Proton Lifetime: Target 3.0e34 years
    tau_p_deviation = (tau_p - tau_p_target_preferred) / (tau_p_central * 0.3) # 30% of base prediction error
    tau_p_unc_penalty = (tau_p_unc_OOM - tau_p_target_error) / 0.1 # Penalty for uncertainty > 0.5 OOM
...
# ==============================================================================
# GRID SEARCH & OPTIMIZATION FUNCTIONS (UNCHANGED)
# ==============================================================================

def grid_search(n_points=41, search_range=10.0):
    """
...
def find_optimal_parameters():
    """
    Use scipy.optimize to find optimal (α_4, α_5) minimizing chi-squared (unconstrained).
    """
...
def compare_predictions(alpha_4_base, alpha_5_base, alpha_4_opt, alpha_5_opt):
    ...
    chi2_base = print_results(0.5, 0.5, "BASELINE")
    
    if alpha_4_opt is not None:
        chi2_opt = print_results(alpha_4_opt, alpha_5_opt, "OPTIMIZED")
        
        # Calculate improvements based on original baseline (0.5, 0.5)
        tau_p_base = compute_tau_p(0.5, 0.5)
        tau_p_opt = compute_tau_p(alpha_4_opt, alpha_5_opt)
        tau_p_unc_base = compute_tau_p_uncertainty(0.5, 0.5)
        tau_p_unc_opt = compute_tau_p_uncertainty(alpha_4_opt, alpha_5_opt)
        alpha_GUT_inv_base = compute_alpha_GUT(0.5, 0.5)
        alpha_GUT_inv_opt = compute_alpha_GUT(alpha_4_opt, alpha_5_opt)
        w_0_base = compute_w_0(0.5, 0.5)
        w_0_opt = compute_w_0(alpha_4_opt, alpha_5_opt)
        theta_23_base = compute_theta_23(0.5, 0.5)
        theta_23_opt = compute_theta_23(alpha_4_opt, alpha_5_opt)
        m_KK_base = compute_m_KK(0.5, 0.5)
        m_KK_opt = compute_m_KK(alpha_4_opt, alpha_5_opt)

        print("IMPROVEMENTS (Relative to Baseline V4):")
        print("-" * 80)
...
if __name__ == '__main__':
    ...
    print("=" * 80)
    df_grid = grid_search(n_points=41, search_range=10.0) 

    # Save grid results
    df_grid.to_csv('extra_dim_tuning_grid_v4.csv', index=False)
    print(f"✓ Grid results saved to 'extra_dim_tuning_grid_v4.csv'")
    print()
    ...
    print("=" * 80)
    print("SUMMARY & RECOMMENDATIONS (V4)")
    print("=" * 80)
    print()
    ...
    if alpha_4_opt is not None:
        ...
        print()
        print("Try exploring grid results in 'extra_dim_tuning_grid_v4.csv'")
