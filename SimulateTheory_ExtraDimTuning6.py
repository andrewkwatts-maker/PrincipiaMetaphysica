...
4. **NEW (V5b): Constrained α₄ and α₅ to [-1.0, 1.0] for a more "natural" fit.**
...
print("=" * 80)
print("PRINCIPIA METAPHYSICA - EXTRA DIMENSION TUNING SIMULATION (V5b)")
print("Constraints: |α₄| ≤ 1.0, |α₅| ≤ 1.0")
print("=" * 80)
print()
...
def objective_function(params):
    ...
    # NOTE: L-BFGS-B handles the bounds on α₄ and α₅, so we don't need a hard penalty here.
...
def grid_search_3d(n_points_2d=21, search_range_2d=1.0, n_points_a6=5, search_range_a6=[0.5, 3.0]):
    """
    Perform a limited 3D grid search over α₄, α₅, and α₆.
    
    NOTE: search_range_2d is now constrained to 1.0
    """
...
def find_optimal_parameters():
    """
    Use scipy.optimize L-BFGS-B to find optimal (α_4, α_5, α_6) minimizing chi-squared
    with explicit bounds on α₄ and α₅.
    """
    print("Finding optimal parameters via L-BFGS-B optimization (3D Bounded)...")
    print()

    # Initial guess aims for good w_0/theta_23 fit and a suppression factor (α₆ > 1)
    # Using a central point (0.8, -0.3) that satisfies the |α| <= 1 constraint
    x0 = [0.8, -0.3, 1.5] 
    
    # Bounds: α₄ in [-1, 1], α₅ in [-1, 1], α₆ in [0.1, 3.0]
    # We set a lower bound of 0.1 for α₆ to avoid numerical issues and enforce positivity
    bounds = (
        (-1.0, 1.0),   # α₄
        (-1.0, 1.0),   # α₅
        (0.1, 3.0)     # α₆ (must be positive, upper bound for search stability)
    )
    ...
    result = minimize(
        objective_function,
        x0,
        method='L-BFGS-B', # Changed method to support bounds
        bounds=bounds,
...
def compare_predictions(alpha_4_base, alpha_5_base, alpha_6_base, alpha_4_opt, alpha_5_opt, alpha_6_opt):
    ...
    """
    print("=" * 80)
    print("COMPARISON: BASELINE vs OPTIMIZED PARAMETERS (Bounded Fit)")
    print("=" * 80)
    print()
    ...
    if alpha_4_opt is not None:
        chi2_opt = print_results(alpha_4_opt, alpha_5_opt, alpha_6_opt, "OPTIMIZED (Bounded)")
...
if __name__ == '__main__':
    ...
    print("=" * 80)
    # Constrained grid search range to [-1.0, 1.0]
    df_grid = grid_search_3d(n_points_2d=21, search_range_2d=1.0, n_points_a6=5, search_range_a6=[0.5, 3.0]) 

    # Save grid results
    df_grid.to_csv('extra_dim_tuning_grid_v5_bounded.csv', index=False)
    print(f"✓ Grid results saved to 'extra_dim_tuning_grid_v5_bounded.csv'")
    print()
    ...
    print("Step 2: Optimization (L-BFGS-B)")
    print("=" * 80)
    ...
    print("=" * 80)
    print("SUMMARY & RECOMMENDATIONS (V5b - Bounded)")
    print("=" * 80)
    print()
    ...
    if alpha_4_opt is not None:
        ...
        print()
        print("Try exploring grid results in 'extra_dim_tuning_grid_v5_bounded.csv'")
