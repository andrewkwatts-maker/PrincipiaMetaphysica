...
class DependencyGraph:
    ...
    def register(self, param: str, depends_on: List[str] = None,
        ...
        Args:
            param: Parameter path (e.g., "geometry.b3")
...
def build_pm_dependency_graph() -> DependencyGraph:
    ...
    graph.register("seeds.b3", depends_on=[],
                   metadata={'level': 0, 'description': 'Dimensional parameter b3 = 24'})
    graph.register("seeds.chi_eff", depends_on=[],
                   metadata={'level': 0, 'description': 'Effective chi = 144'})
    graph.register("seeds.christ_constant", depends_on=[],
    ...
    graph.register(
        "geometry.k_gimel",
        depends_on=["seeds.b3"],
        compute_fn=lambda deps: 2 * 3.141592653589793 / deps["seeds.b3"],
    ...
    graph.register(
        "geometry.V_G2",
        depends_on=["seeds.b3"],
        compute_fn=lambda deps: deps["seeds.b3"] ** 1.5,
    ...
    graph.register(
        ...
        compute_fn=lambda deps: 1.0 / deps["seeds.chi_eff"],
    ...
    graph.register(
        ...
        depends_on=["seeds.b3"],
        compute_fn=lambda deps: 3.0 / (8.0 + deps["seeds.b3"]),
    ...
    graph.register(
        ...
        depends_on=["geometry.k_gimel", "seeds.chi_eff", "seeds.christ_constant"],
        compute_fn=lambda deps: 71.55 * (deps["geometry.k_gimel"] * deps["seeds.chi_eff"] / deps["seeds.christ_constant"]) ** 0.1,
    ...
    graph.register(
        ...
        depends_on=["seeds.b3"],
        compute_fn=lambda deps: -1.0 + 1.0 / deps["seeds.b3"],
