...
### The adoption sweep completes: every simulation on the ruled path, every cost measured

**2026-09-22, the 24-to-43 sweep.** Directive: find every old design on the
24 seed, switch it where a switch belongs, replace it where it does not, and
keep everything runnable. Executed with a census (1330 B3_MOVES / 639
BULK_STAYS / 47 AMBIGUOUS across 183 files, artifact b3_24_census.json),
three parallel worker passes over 26 test files, and per-module src
corrections. Every re-pinned value was MEASURED by running the code; no
tolerance was widened; nothing was deleted.

#### The costs of the ruling, now numbers on the books

| quantity | seed_24 | adopted seed | verdict |
|---|---|---|---|
| eta_B (baryogenesis) | 6.185e-10, 1.63 sigma | 6.846e-10, **18.16 sigma** | recorded divergence |
| axion g_a_gamma | in BabyIAXO window | **2.69e-11 > 2e-11 ceiling** *if the module followed the seed* -- it does NOT yet | NOT a published cost; see the correction below |
| theta_13 (flavour fork) | 8.6686 deg (0.8 sigma) | 4.8351 deg | recorded divergence |
| ancestral_roots n(n-1)/2+12 | 288 | **915** | "SO(24) generators" label false on-path |
| h0_local | 71.2480 | 71.7396 | the gap IS the chi_eff dichotomy |
| sterile = bulk (163 = 7b3-5) | held | 163 vs 296 | a seed_24 coincidence, permanently recorded |
| continuous parameter count | 22 | **23** | wa_thawing returned when its -4/sqrt(b3) removal stopped reproducing |

#### The neutrino tree's cosmetic dependency, removed

`neutrino_sector` multiplied its m_lightest tree by b3_leaf()*(1/24) -- "an
identity factor" -- purely so a dependency tracer would see a b_3 reference.
Only an identity at 24: on the adopted seed the tree silently computed
m_lightest * 43/24. m_lightest does not ride on b_3, and faking provenance
to please a tracer is the same conflation class the whole sweep exposed. The
tree now states the honest dependency: none.

#### New geometry: 128 conical assignments exist in the shift family

The flavour wall moved. Scanning all 2^21 generator-shift assignments
(composite shifts derived by the cocycle): **128 admit a full-group fixed
point** -- a codimension-7 conical singularity with transverse cone
R^7/(Z/2)^3. All 128 are OFF the A1-admissible branch (all-7-singular is
the opposite of disjointness), so none carries (12, 43); the zero-shift
orbifold anchors the criterion, and the canonical shifts correctly return
none. Three questions stay open and are stated in the module: whether such
cones support chiral states, whether the orbifolds admit a G2 resolution,
and whether any deformation connects the branches. The boundary statement
shrank from "the family has no conical points" to "the family has 128,
resolution physics open".
...
never as edits to the adopted path.

#### Named debts remaining (each awaiting its pass)

Seed-blind module defaults still calibrated at 24 behind the divergence
ledger: axion_photon_coupling.DEFAULT_B3, vacuum_selection.DEFAULT_B3,
gate G22's 24/288, gate G01's 288, switch_search's abandoned b_3/8 probe.
appendix_h's "276 + 24 - 12 = 288" prose vs its emitted 915. baryogenesis'
hardcoded "within 1.1% (2.2 sigma)" prose vs the measured 18.16. The
artifact search order prefers a sibling checkout's AutoGenerated over the
repo-local one. The validation_report placeholder made 7 tests pass
vacuously; they now skip with the actionable reason.

#### Search directions opened by the sweep

(A) racetrack exponent pairs from the DERIVED integer menu {3,4,7,12,24,26,
36,43} -- e.g. (7,12) restores the lost ordering -- as a costed enumeration
feeding re_t_adoption; (B) O(t^2) K_IJ, since the flux null was a
leading-order statement; (C) an identity ledger evaluating every claimed
integer identity per fork state, so the next broken coincidence is a report,
not a crash; (E) a fork-implication matrix publishing {moves/breaks/heals}
per one-fork deviation.

#### The shadow audit's two-way verdict

The shadow-derivation audit (itself two-directional, like the divergence
ledger) reported both directions of the ruling at once: **three recorded
conflicts HEALED** -- H0_local, S8 and sigma_m_nu, whose rival derivations
moved together with the seed into tolerance -- and **one new cost**:
higgs.m_higgs_local = 70.5675 against pdg 125.2 (43.64%), the b_3-consuming
VEV formula's 24-calibration not surviving the ruling. Removed and added per
the audit's own rule; the healed entries' history stays in the register
(1.7, 1.4, 1.5), and a returning conflict would be a new measurement, not a
resurrection.

#### The defect the sweep's own artifacts exposed: 61 formulas were vanishing

Registering a formula ran `triple_assert` and let an AssertionError
propagate. Under the adoption that raise started firing -- correctly, on
the ruled-divergence rows -- but it fired MID-REGISTRATION, so every
formula a simulation had not yet registered silently disappeared from the
published artifacts. Measured: **508 of 569 formulas published**, 22
parameters naming a `derivation_formula` that no longer existed, and the
site omitting exactly the rows it is supposed to display as honest FAILs.
The gate was deleting the evidence it exists to show.

Fixed at the registration site: a formula listed in
`core/ruled_divergences.py` registers WITH `triple_status =
RULED_DIVERGENCE` and its recorded reason; a formula OUTSIDE the ledger
still raises, so the gate keeps its teeth. Rebuilt: **569 published, 0
dangling**. This is the sharpest argument yet for the two-directional
ledger: without a place to record "this diverges by record", the only
available behaviour was to hide the row.

#### Both gate pins retired the same day they were written

G22's dual pin (live FAIL, shipped PASS) existed because the published
bundle was half-republished; the rebuild synchronised it, both sides now
read COMPUTED_FAIL honestly (43/288 = 0.149306 against the gate's own
24/288), and the pin was DELETED per its own design note rather than
widened. G17's chi_eff exclusion retired with the defect that caused it:
`FormulasRegistry.n_gen` no longer routes through `chi_eff // b_3`
(= 72 // 43 = 1) but resolves the RULED `n_gen_source` fork, so G17
evaluates COMPUTED_PASS from a ruled route with no chi_eff dependence.
All eight semantic evaluators are compared again.

#### Coverage ratchets re-measured, in both directions

arithma b3-rooted 381 -> **380**: one formula legitimately stopped rooting
through b_3, because `b3-generations` was rewired to the ruled n_gen =
b_2/4. EML b3-rooted 109 -> **134** (and non-b3 460 -> 435): the flavour
trees now resolve through one fork-aware leaf, so the walker sees paths a
frozen literal had hidden. The two walkers' disagreement narrowed 3.6x ->
**2.836x**, and its test -- which previously asserted `ratio > 3.0` and
told its reader to tighten when they converged -- now ratchets DOWNWARD,
with a floor that retires it when the instruments agree.


#### A correction to this entry, made by the seed-blindness detector

The axion row above was first written as a published cost. It is not one,
and the distinction matters more than the number: `particle.
axion_photon_coupling_status` reads "lies within BabyIAXO/IAXO discovery
window" **identically under both seeds**, and `alp.coupling_GeV_inv` is
frozen display prose. The 2.69e-11 figure is what the coupling WOULD be if
`axion_photon_coupling.DEFAULT_B3` followed the seed like the rest of the
flavour sector -- measured by calling the class directly, which is a
prediction about a pending fix, not a reading of the published pipeline.
It stays on the books as the expected consequence of debt (a), and becomes
a published cost only when that debt closes.

The same detector CONFIRMED two costs that were in doubt for the same
reason: `cosmology.eta_B` genuinely MOVES (6.185e-10 -> 6.846e-10, the
18.16 sigma) and `higgs.m_higgs_local` genuinely MOVES (125.102 ->
70.568). Those are physics on the adopted path, not frozen values wearing
a result.

And it sharpened the racetrack finding: `cosmology.racetrack_Re_T` is
STATIC at 7.086 across both seeds, because the published row takes the
calibrated fallback when the solver finds no minimum. The deletion of the
vacuum is real in the solver and defended by tests; what the PUBLISHED row
shows is the fallback, not the absence. A reader of parameters.json alone
cannot see that the vacuum died -- which is its own reporting defect, now
named.

#### The detector itself: core/seed_blindness.py

Four seed-blind writers were found one at a time, each by a different
accident -- a fork that could not propagate, a provenance guard, a
re-measurement agent, an ambiguous-alias guard. Four instruments catching
four instances of ONE class means the class was never searched for. The
detector runs the real registration path under both seeds in separate
processes (a per-process singleton makes in-process flipping meaningless)
and classifies every row: **786 compared, 182 MOVED, 135 FROZEN_CLAIM,
469 static**. A FROZEN_CLAIM is a suspect for a reading, not a proven
defect -- `geometry.D_bulk` and friends are supposed to stay put. The
ratchet is on the count, and it may fall but never rise.

This is the argument for switches stated as a method: because both branches
are runnable, "did this conclusion come from a bug?" is an EXPERIMENT --
run both, diff, and the frozen rows announce themselves -- rather than an
audit of intentions. Every headline of the adoption was put through it
above, and one of them did not survive.

#### A second publishing gap, found by auditing the ledger itself

Checking the ruled-divergence ledger for rows naming formulas nobody
publishes turned up six -- and widening the question found the shape behind
them. The triple-track collector walks **637** formulas; `formulas.json`
carries **569**. The 68-formula gap is spread evenly over ~20 modules (4-6
each), which is the signature of whole simulations missing from the
publishing path rather than individual formulas being dropped.

This is a DIFFERENT defect from the mid-registration truncation closed the
same day. That one lost 61 formulas from simulations that did publish and
is fixed (569 published, 0 dangling). This one asks whether every
simulation defining formulas is in the published run at all, and it needs a
reading per module rather than a sweep. Measured, ratcheted
(`test_published_formula_coverage.py`, ceiling on the GAP so a newly
published formula lowers it), triage outstanding.

Worth stating why it survived: a test asserting "569 are published" would
have passed happily forever. Only a test comparing the two SIDES could see
it. The same shape as the two-directional gates -- a count is not a check
unless something can make it move in both directions.

#### The fifth seed-blind writer, and the guard that caught it

`vacuum_selection.DynamicalVacuumSelector.DEFAULT_B3 = 24` was still
publishing `cosmology.b3 = 24` while `particle.b3` carried the adopted 43 --
two rows, same short name, different values. The ambiguous-alias guard
caught it (an instrument built for a different purpose entirely, which is
the pattern this defect class keeps showing). The frozen default is gone;
resolution happens at call time via the seed fork, and both rows now read
43 in the published artifact.

#### The seed fork stops being two hand-written entries

`b3_path.PATHS` listed seed_24 and seed_43_joyce BY HAND while the
contribution table derives FOUR reachable profiles (b_3 = 7 + 3 n_T3 over
n_T3 in {0, 4, 8, 12}, b_2 = n_T3). So the load-bearing claim behind the
adoption -- "n_gen = rank(Gamma) = 3 selects (12, 43) uniquely out of four
enumerated candidates" -- was an argument about candidates the pipeline
could not execute. Two of the four existed only in prose.

PATHS is now GENERATED from that relation, all four are runnable forks, and
the selection is MEASURED end to end:
...
Only the adopted profile gives three, and the other three fail
STRUCTURALLY -- a generation count is a number of things, so 0, 1 and 2 are
refutations of a different kind from a disagreement with data. The registry
resolves each seed with no special casing (elder_kads reads 7/19/31/43),
which is what makes a candidate costable rather than arguable.
`test_reachable_family_is_runnable.py` checks the selection by running it,
and states the relation independently so it is not checking the generator
against itself. seed_24 stays runnable and stays LABELLED unreachable,
with a test asserting it still fails b_3 = 7 + 3 b_2 -- keeping it is the
point, and so is not pretending it is on the family.

#### A process note, because it cost real work

Restoring a heredoc-corrupted `variants.py` with `git checkout --` threw
away every uncommitted edit in that file -- the flavour fork, the
b3_origin re-scope. Exactly the loss recorded earlier this campaign for the
`metric_construction` fork, repeated because the same shortcut was taken.
Both were rebuilt by running a patch script WRITTEN TO A FILE, which is
the rule that should have been followed first: variants.py is never edited
through a shell heredoc, because the escapes are interpreted twice and the
file is large enough that the damage is not obvious.

#### The selection argument gets SHARPER when the family becomes runnable

`compare_paths` used to report `both_pass_structurally = True` with the
reading "both paths give three generations, so the structural test does NOT
decide between them -- the ruling is genuinely the author's". With only two
paths declared that was correct. With the family runnable it is incomplete,
and the corrected statement is stronger:

**The structural test DOES select -- within the family.** It eliminates the
profiles giving 0, 1 and 2 generations and leaves exactly one. What it does
not do is separate (12, 43) from the off-family seed_24, which also gives
three via 24/8. That separation is REACHABILITY: seed_24 fails b_3 = 7 +
3 b_2, fails b_3 = 7 (mod 12), and sits below the exhibited TCS range.

So the adoption rests on TWO independent criteria, neither referencing a
measurement:

  generation count   selects uniquely WITHIN the reachable family
  reachability       excludes the off-family candidate that also gives three

`compare_paths` now reports both (`reachable_paths_passing_structural`,
`structural_test_is_decisive_within_the_family`,
`what_separates_the_two_that_pass`), and the tests assert each separately
so a future change that weakens one cannot hide behind the other.

#### Search priority, and the defect that made it necessary

`VariantOption` gained a declared `priority` (0 = the live alternative worth
costing early, 1 = plausible, 2 = retained-for-completeness because a
falsified candidate stays runnable and labelled), and `combinations_for`
now enumerates ADOPTED FIRST, then by priority, then declaration order.

The defect that forced it: enumeration was declaration order, which was
harmless while forks had two options and broke the moment `b3_seed` grew to
five. A `cap=3` sweep explored seed_24, seed_7_joyce, seed_19_joyce and
NEVER REACHED the adopted seed_43_joyce -- a truncated search silently
...
**And the first fix for this was itself a defect, caught the same way the
rest of the pass caught things — by looking rather than assuming.**
`METAPHYSICA_OUT` is the WRITE target as well as the read hint, so pointing
it at the published checkout let generator write paths reach the real
artifacts: one full suite run silently removed **fourteen rows** from the
published `parameters.json` and rewrote seven other artifacts. Found by
checking `git status` in the published checkout after the run, not by any
test. The suite now stages a COPY of the artifacts in a per-session scratch
directory and writes there; a test asserts the session's write target is
never the published tree.
