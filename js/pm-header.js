...
/**
 * Ticker state - formulas loaded dynamically from JSON
 */
let tickerFormulas = [];
let tickerIndex = 0;
...
/**
 * Load formulas from AutoGenerated/formulas.json
 * Returns array of {category, latex, title} objects
 */
async function loadTickerFormulas() {
  const basePath = getBasePath();
  const paths = [
    `${basePath}AutoGenerated/formulas.json`,
  ...
  for (const path of paths) {
    try {
      ...
      if (response.ok) {
        ...
        // Convert formulas object to array, filter for ones with good LaTeX
        const formulas = Object.values(data.formulas || {})
          .filter(f => f.latex && f.latex.length > 5 && f.latex.length < 120)
          .map(f => ({
            category: f.category || 'Formula',
            latex: f.latex,
            title: f.title || f.description || ''
          }));

        // Shuffle and take first 20 for variety
  ...
  return [
    { category: 'Status', latex: '\\mathbf{72/72}\\text{ Gates Locked}', title: 'All gates validated' }
  ];
}
...
/**
 * Initialize the theory ticker - displays rotating LaTeX formulas from formulas.json
 * Changes every 10 seconds with a fade transition, renders with MathJax
 */
async function setupTicker() {
  ...
  tickerFormulas = await loadTickerFormulas();
  if (tickerFormulas.length === 0) {
    ticker.style.display = 'none';
    return;
  }

  // Display current formula with MathJax rendering
  function showFormula(index) {
    const formula = tickerFormulas[index];
    ticker.classList.add('fade-out');

    setTimeout(() => {
      categoryEl.textContent = formula.category;
      categoryEl.className = 'ticker-category cat-' + formula.category.toLowerCase();
      // Set title attribute for hover tooltip
      textEl.title = formula.title;
      // Wrap in LaTeX inline delimiters for MathJax
      textEl.innerHTML = `\\(${formula.latex}\\)`;
      ticker.classList.remove('fade-out');

      // Trigger MathJax to render the new formula
      if (window.MathJax && window.MathJax.typesetPromise) {
        window.MathJax.typesetPromise([textEl]).catch((err) => {
          console.warn('[PM Header] MathJax error:', err);
        });
      }
    }, 300);
  }
  ...
  tickerInterval = setInterval(() => {
    tickerIndex = (tickerIndex + 1) % tickerFormulas.length;
    showFormula(tickerIndex);
  }, 10000);
