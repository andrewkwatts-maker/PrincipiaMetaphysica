...
def run_post_processing():
    ...
    # 8. Extract supplementary content
    print("\n8. Extracting supplementary content...")
    supp_script = project_dir / "simulations" / "Constants" / "extract_supplementary.py"
    if supp_script.exists():
        result = subprocess.run(
            [sys.executable, str(supp_script)],
            cwd=str(project_dir),
            capture_output=True,
            text=True
        )
        for line in result.stdout.split('\n'):
            if '[OK]' in line or '[SUCCESS]' in line:
                print(f"   {line.strip()}")
        if result.returncode != 0:
            print(f"   WARNING: Supplementary extraction failed - {result.stderr[:100]}")
    else:
        print("   SKIP: extract_supplementary.py not found")
    ...
    print("  - AutoGenerated/supplementary.json (3 items)")
    print("=" * 70)
