...
# Note: simulations excluded for smaller website-only package
...
def copy_wolfram_results():
    """Copy wolfram_results.json from simulations/AutoGenerated to AutoGenerated for offline use."""
    print("\n  Copying Wolfram results for offline access...")

    src = SOURCE_DIR / "simulations" / "AutoGenerated" / "wolfram_results.json"
    dst = BUILD_DIR / "AutoGenerated" / "wolfram_results.json"

    if src.exists():
        dst.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(src, dst)
        print(f"    Copied: wolfram_results.json to AutoGenerated/")
    else:
        print(f"    [WARN] wolfram_results.json not found - Wolfram validation may not work offline")


def copy_foundations_registry():
    """Copy foundations_registry.json from simulations/base to AutoGenerated for offline use."""
    src = SOURCE_DIR / "simulations" / "base" / "foundations_registry.json"
    dst = BUILD_DIR / "AutoGenerated" / "foundations_registry.json"

    if src.exists():
        dst.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(src, dst)
        print(f"    Copied: foundations_registry.json to AutoGenerated/")
    else:
        print(f"    [WARN] foundations_registry.json not found - Foundations page may not work offline")
...
def create_offline_config():
    ...
    # Overwrite site-config.js with offline settings
    site_config_js = BUILD_DIR / "js" / "site-config.js"
    site_config_js.parent.mkdir(parents=True, exist_ok=True)

    with open(site_config_js, 'w', encoding='utf-8') as f:
        f.write('''/**
...
window.PM_CONFIG.VERSION = '%s';
window.PM_CONFIG.RELEASE_DATE = '%s';
...
console.log('[PM Config] Zenodo Offline Release v%s - No Auth Required');
''' % (VERSION, RELEASE_DATE, VERSION))

    print(f"    Updated: js/site-config.js (offline mode enabled)")
...
Examples:
  ...
  python scripts/zenodo_pack_v16.py --full --validate  # Full package with validation
    ...
    # Step 3b: Copy data files for offline use
    copy_wolfram_results()
    copy_foundations_registry()
