...
def so10_anomaly_cancellation():
    """
    SO(10) with 3x16 + singlets
    Anomaly coefficient A = Tr(T^a {T^b, T^c}) = n_16 x 1 + n_s x 0
    For 3 generations: A = 3 -> cancels with Green-Schwarz term Delta=3
    """
    n_gen = 3
    A_16 = n_gen * 1
    A_s = 0
    total = A_16 + A_s
    GS_term = 3  # from axion in G_2 compactification
