...
Update shadow_kuf and shadow_chet parameters to match NuFIT 6.0 data
...
def update_alpha45_nufit6(theta23_nufit6=45.0, delta_theta=0.0):
    """
    Update shadow_kuf and shadow_chet to match NuFIT 6.0 theta_23
    ...
    Returns:
        shadow_kuf, shadow_chet, theta_23_predicted
    """
    ...
    # Relationship: theta_23 ≈ 45° + (shadow_kuf - shadow_chet) * n_gen
    ...
    # Solve for shadow_kuf and shadow_chet
    shadow_kuf = (alpha_sum + alpha_diff) / 2
    shadow_chet = (alpha_sum - alpha_diff) / 2

    # Predicted theta_23
    theta_23_pred = 45.0 + (shadow_kuf - shadow_chet) * n_gen
    ...
    print(f"  shadow_kuf + shadow_chet = {alpha_sum:.6f} (geometric)")
    print()
    print("NuFIT Updates:")
    ...
    print(f"  shadow_kuf = {shadow_kuf:.6f} (was 0.8992)")
    print(f"  shadow_chet = {shadow_chet:.6f} (was -0.3823)")
    print(f"  shadow_kuf - shadow_chet = {shadow_kuf - shadow_chet:.6f}")
    print()
    print("Predictions:")
    ...
    return shadow_kuf, shadow_chet, theta_23_pred

def explore_uncertainty_range():
    """Explore shadow_kuf/shadow_chet over NuFIT 6.0 uncertainty range"""
    ...
    for delta in [-1.5, 0.0, +1.5]:
        ...
        print(f"theta_23 = {theta23:5.1f}°: shadow_kuf = {a4:.6f}, shadow_chet = {a5:.6f}")

if __name__ == "__main__":
    # Central value update
    shadow_kuf, shadow_chet, theta_23 = update_alpha45_nufit6(45.0)
    ...
    print("- Update config.py: shadow_kuf = {:.6f}, shadow_chet = {:.6f}".format(shadow_kuf, shadow_chet))
