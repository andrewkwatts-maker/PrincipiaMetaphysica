    ...
    def get_formulas(self) -> List[Formula]:
        ...
        return [
            Formula(
                ...
                label="(1.1)",
                latex=r"S = \int d^{26}X \sqrt{-G} \left[ R + \bar{\Psi}_P (i \Gamma^M D_M - m) \Psi_P + \lambda (\bar{\Psi}_P \Psi_P)^2 + g \, t_{\text{ortho}} \, \bar{\Psi}_P \Psi_P + \mathcal{L}_{\text{Sp}(2,\mathbb{R})} \right]",
                plain_text="S = integral d^26X sqrt(-G) [ R + Psi-bar_P (i*Gamma^M*D_M - m) Psi_P + lambda*(Psi-bar_P*Psi_P)^2 + g*t_ortho*Psi-bar_P*Psi_P + L_Sp(2,R) ]",
                category="THEORY",
                description=(
                    "26D Pneuma master action in (24,2) signature. Unifies gravity (R), "
                    "fermionic matter (8192-component Pneuma spinor), quartic self-interaction, "
                    "orthogonal time coupling, and Sp(2,R) gauge fixing."
                ),
                inputParams=[],
                outputParams=[],
                terms={
        ...
        return SectionContent(
            ...
            content_blocks=[
                ...
                ContentBlock(
                    type="paragraph",
                    content=(
                        "The fundamental action begins in 26D spacetime with signature (24,2), "
                        "unifying gravity and fermionic matter via the Pneuma spinor. The Sp(2,R) "
                        "gauge fixing reduces to 13D, then Kaluza-Klein compactification over G2 "
                        "yields the effective 4D theory with Standard Model gauge sectors."
                    )
                ),
                ContentBlock(
                    type="formula",
