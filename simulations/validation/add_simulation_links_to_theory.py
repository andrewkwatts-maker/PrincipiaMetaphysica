...
def update_theory_output(repo_root: Path = None):
    ...
    if repo_root is None:
        ...
        while current.parent != current:
            if (current / 'AUTO_GENERATED').exists():
                repo_root = current
                break
            current = current.parent
        else:
            raise RuntimeError("Could not find AUTO_GENERATED directory")

    theory_json_path = repo_root / 'AUTO_GENERATED' / 'theory_output.json'
