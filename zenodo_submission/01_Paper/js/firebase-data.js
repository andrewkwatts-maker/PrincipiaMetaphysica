...
/**
 * Fallback to load static theory-constants-enhanced.js
 * @returns {Promise<Object>} PM object from static file
 */
async function loadStaticFallback() {
  console.log('[PM Data] Falling back to static theory-constants-enhanced.js');

  try {
    // Try to load the static file
    const response = await fetch('/theory-constants-enhanced.js');
    const text = await response.text();

    // Extract the PM object (it's assigned to const PM = {...})
    // This is a simple extraction - the static file defines PM globally
    const script = document.createElement('script');
    script.textContent = text;
    document.head.appendChild(script);

    if (window.PM) {
      console.log('[PM Data] Static fallback loaded successfully');
      dataCache.theoryConstants = window.PM;
      dataCache.lastUpdated = Date.now();
      return window.PM;
    }
  } catch (error) {
